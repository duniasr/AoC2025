...
* **SOLID**:
    ...
    * **Interface Segregation Principle (ISP)** *(No forzar a depender de interfaces que no se usan)*: La interfaz `PresentsFit` es estrictamente específica y cuenta con un único método (`fitUnderRegion`), garantizando que la clase [`Main`][main] no tenga que depender de métodos irrelevantes.
    * **Dependency Inversion Principle (DIP)** *(Depender de abstracciones, no de implementaciones)*: La clase [`Main`][main] y los tests operan sobre la interfaz `PresentsFit`, sin depender de la compleja implementación concreta.
...
* **You Aren’t Gonna Need It (YAGNI)** *(No lo vas a necesitar)*: Hemos creado lo mínimo y estrictamente necesario (`TreeRegion` y `PresentShape`) para resolver el problema, evitando de la sobreingeniería.
...
* **Genéricos** *(Parametrización de tipos para reutilización segura)*: Uso robusto del tipado en la serialización como `List<Point>` y la orquestación a través de `Stream<TreeRegion>`.
* **Good Naming** *(Nombres descriptivos y precisos)*: En lugar de usar nombres técnicos abstractos, usamos términos del problema real que explican claramente lo que hacen (ej: `PresentsFitLogic`, `fitUnderRegion`). Además, los métodos privados se leen como preguntas naturales (`isInside`, `allPositionsIn`, `maskAt`) y se ha erradicado por completo el uso de variables de una sola letra (cambiando parámetros opacos por descriptivos como `region`, `shapePoints`, `occupied`, `mask` o `index`).
...
* **Monad (Funcional)** *(Estructura compositiva aplicable a colecciones anidadas)*: Uso del patrón monádico a través de `flatMap` para aplanar transformaciones profundas en [`PresentShape`](PresentShape.java).
...
* **Programación Funcional** *(Estilo declarativo basado en funciones puras y datos inmutables)*: Predominio total del paradigma, eliminando el control de flujo imperativo (ausencia de loops explícitos) y apostando por tuberías declarativas y recursividad.
