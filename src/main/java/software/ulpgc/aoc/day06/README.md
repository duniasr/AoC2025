...
* **Encapsulamiento** *(Ocultación del estado interno y protección de los datos)*: La entidad `MathProblem` aísla internamente sus datos (la lista de números y el operador a aplicar). El exterior no necesita saber cómo se iteran esos números; simplemente llama al método público `solve()` y obtiene el resultado.
...
* **Don't Repeat Yourself (DRY)** *(Evitar la duplicación de lógica)*: `Operator` centraliza tanto el símbolo matemático original como su operación correspondiente. Además, se reutiliza en ambas partes (A y B) sin duplicar la lógica matemática.
* **Composition Over Inheritance (COI)** *(Preferir componer sobre heredar)*: En lugar de crear una jerarquía de clases usando herencia (ej. crear una clase base `MathProblem` y subclases `AddMathProblem` y `MultiplyMathProblem`), hemos preferido componer el objeto agregándole el comportamiento como un atributo (`Operator operator`). Esto hace el diseño infinitamente más flexible.
