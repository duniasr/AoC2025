...
## Lógica Estructural
* **`Coordinate`**: [`Coordinate`](Coordinate.java) - Representa un punto en el espacio 2D y encapsula la lógica geométrica para generar un flujo (`Stream`) de sus propias coordenadas adyacentes.
* **`PaperRollDiagram (A)`**: [`PaperRollDiagram`](a/PaperRollDiagram.java) - Representación inmutable del mapa. Actúa exclusivamente como entorno de datos de solo lectura, procesando el array de texto y exponiendo métodos de consulta.
* **`PaperRollDiagram (B)`**: [`PaperRollDiagram`](b/PaperRollDiagram.java) - Representación mutable del mapa. Actúa como el entorno, utilizando una matriz interna de caracteres para permitir consultas y la alteración del estado local (`removeRoll`).
...
* **Escaneo de Vecindad (Moore Neighborhood):** Algoritmo de detección espacial que calcula la densidad de rollos en las 8 direcciones adyacentes, aplicando restricciones de límites para evitar excepciones de acceso a memoria fuera de rango ([`Coordinate`](Coordinate.java)).
...
* **Composition Over Inheritance (COI)** *(Preferir componer sobre heredar)*: El modelo `PaperRollDiagram` no intenta heredar de estructuras nativas bidimensionales; en su lugar, encapsula y compone una cuadrícula (`String[] grid`) para manejar sus propias reglas espaciales.
* **Keep It Simple, Stupid (KISS) & You Aren't Gonna Need It (YAGNI)** *(Simplicidad y no añadir código innecesario)*: El problema se resuelve a la perfección con clases directas y un simple filtrado secuencial de Streams.
...
* **Métodos Delegados** *(Dividir tareas complejas y delegar sub-operaciones)*: El conteo de rollos accesibles en [`Forklift (A)`](a/Forklift.java) delega sus filtros a los predicados como (`diagram::isRoll`).
* **Inyección de Dependencias** *(Pasar colaboradores/datos en los parámetros)*: El actor `Forklift` recibe el `PaperRollDiagram` inyectado por parámetro en su método principal, desacoplándolo de la creación del mapa.
...
* **Parte A:** [`aTest`](../../../../../../test/java/test/day04/aTest.java) - Valida que se cuenten correctamente los rollos accesibles iniciales en el mapa de prueba (resultado esperado = `5`).
* **Parte B:** [`bTest`](../../../../../../test/java/test/day04/bTest.java) - Simula la extracción recursiva en cascada y valida que el total histórico de rollos retirados coincida con la especificación (resultado esperado = `12`).
