...
* **SOLID**
    ...
    * **Open/Closed Principle (OCP)** *(Abierto a la extensión, cerrado a la modificación)*: Si mañana queremos usar una base de datos u otro algoritmo en lugar del grafo actual (`UnionFind`), podemos crear una clase nueva que implemente la interfaz `ElectricalGrid`. Como la clase principal (`Playground`) solo depende de esa interfaz, podrá usar el nuevo algoritmo inmediatamente sin tener que modificar su código fuente.
    * **Liskov Substitution Principle (LSP)** *(Los subtipos deben ser sustituibles por sus tipos base)*: La clase `UnionFindElectricalGrid` implementa la interfaz `ElectricalGrid`. El principio se cumple porque cualquier código cliente (como la clase `Playground`) que consuma la interfaz, funcionará perfectamente y sin sorpresas al recibir esta implementación, ya que cumple rigurosamente con los contratos de conectividad.
