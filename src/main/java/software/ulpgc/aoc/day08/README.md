...
## Lógica Estructural
* **`Playground`**: (Parte A: [Playground.java](a/Playground.java) / Parte B: [Playground.java](b/Playground.java)) - Toma las decisiones de conexión basándose en las longitudes de los cables y dirige el flujo de ejecución.
* **`ElectricalGrid`**: [ElectricalGrid.java](ElectricalGrid.java) - Interfaz matemática abstracta. Define el contrato estricto para fusionar circuitos y obtener sus tamaños.
* **`UnionFindElectricalGrid`**: [UnionFindElectricalGrid.java](UnionFindElectricalGrid.java) - Implementación concreta de la red eléctrica. Responsable de la lógica matemática subyacente para agrupar redes eficientemente (mediante la estructura de datos *Disjoint-Set*).
* **`StringOfLights`**: [StringOfLights.java](StringOfLights.java) - Entidad inmutable (`record`) portadora de datos que relaciona dos cajas y conoce la distancia que las separa.
* **`JunctionBox`**: [JunctionBox.java](JunctionBox.java) - Entidad inmutable (`record`) del dominio físico que conoce su ubicación espacial en 3D y calcula distancias geométricas.
...
* **Law of Demeter (LoD)** *(Evitar acoplamiento ordenando acciones en lugar de consultar estado interno)*: En [StringOfLights.java](StringOfLights.java) el cable se autoinicializa pidiéndole a la caja `a` calcular la distancia a `b` (`a.distanceTo(b)`) en lugar de extraer sus componentes numéricos cartesianos.
...
* **Parte A:** [aTest.java](../../../../../../../test/java/software/ulpgc/aoc/day08/aTest.java) - Valida que tras 10 conexiones cortas el producto del tamaño de los 3 mayores circuitos sea correcto (resultado esperado = `40`).
* **Parte B:** [bTest.java](../../../../../../../test/java/software/ulpgc/aoc/day08/bTest.java) - Valida la conexión total hasta lograr un único circuito y calcula el producto de las coordenadas del último cable instalado (resultado esperado = `14136`).
