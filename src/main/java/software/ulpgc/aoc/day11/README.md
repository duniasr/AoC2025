...
* **Genéricos** *(Parametrización de tipos para reutilización segura)*: Uso riguroso y fuerte del tipado de colecciones complejas sin castings crudos: `Map<String, List<String>>`.
...
## Paradigmas
* **Orientación a Objetos** *(Organización del software en objetos que encapsulan estado y comportamiento)*: Representado a través de un encapsulamiento puro del conocimiento topológico en la entidad de estado [`Reactor`](Reactor.java), ocultando su manipulación cruda detrás del director de negocio [`PathCounting`](a/PathCounting.java).
* **Programación Funcional** *(Estilo declarativo basado en funciones puras y datos inmutables)*: El núcleo algorítmico matemático ([`PathFinder`](PathFinder.java)) es la máxima expresión de estilo funcional puro: una función matemática estricta sin absolutamente ningún *side-effect* mutable al sistema global externo, sostenida de manera declarativa sobre un `record` de solo-lectura y *pipelines* reductoras.
