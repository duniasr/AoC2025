...
* **Factory Method (Creacional)** *(Encapsulación de la creación de objetos en métodos estáticos dedicados)*: `Rotation.from(instruction)` ([Rotation.java](Rotation.java)) y `Dial.initial()` ([Dial.java](a/Dial.java)) actúan como factorías para aislar la instanciación de objetos válidos del cliente.
